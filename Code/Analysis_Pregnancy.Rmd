...
p <- ggplot(data = top_df, aes(x = coord , y = intensity, fill = Group)) +
  ...
  ylab('Abundance')
ggsave('..//Analysis//Pregnancy//Boxplots.png', plot = p, width = 3.25, height = 2.75)
...
p <- ggplot(data = top_df, aes(x = coord , y = intensity, fill = Group)) +
  ...
  theme(axis.text = element_text(size = 16),
        ...
        legend.text = element_text(size = 12),  # Adjust the legend text size (smaller)
        legend.key.size = unit(0.5, "cm"),
        legend.title = element_text(size = 16)) +  # Adjust the legend title size
  theme(plot.title = element_text(hjust = 0.5)) +# Center title
  xlab('Coordinate (m/z, drift time)') +
  ylab('Abundance') 
ggsave('..//Analysis//Pregnancy//Boxplots_slides.png', plot = p, width = 10, height = 5)
